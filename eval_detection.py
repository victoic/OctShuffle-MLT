...
def load_annotation(filelist, icdar2013=False):
  ...
  for line in list_file:
    ...
    with open(gt_path, 'r') as gt:
      ...
      if len(gt_images) == 500:
        yield np.array(gt_images), np.array(gt_boxes), fns
        gt_images = []
        gt_boxes = []
        fns = []
  yield np.array(gt_images), np.array(gt_boxes), fns

def eval_detection(opts, images, gt_boxes, fns, net=None):
  if net == None:
...
def batch_eval(args):
  iterator = load_annotation(args.eval_list, args.icdar2013)
  true_positives=0
  false_positives=0
  false_negatives=0
  try:
    while True:
      images, gt_boxes, fns = next(iterator)
      tp, fp, fn = eval_detection(args, images, gt_boxes, fns)
      true_positives+=tp
      false_positives+=fp
      false_negatives+=fn
  except:
